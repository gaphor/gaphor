...
def get_connected(item, handle) -> Optional[Presentation[UML.Element]]:
    """
...
class MessageLifelineConnect(BaseConnector):
    ...
    def disconnect(self, handle):
        line = self.line
        send: Optional[Presentation[UML.Element]] = get_connected(line, line.head)
        received = self.get_connected(line.tail)
...
class ExecutionSpecificationExecutionSpecificationConnect(BaseConnector):
    ...
    def connect(self, handle, _port):
        ...
        connected_item: Optional[Presentation[UML.Element]]
