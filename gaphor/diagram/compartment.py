...
class CompartmentItem(NamedItem):
    ...
    def __init__(self, id=None):
        ...
        self.watch("subject.appliedStereotype", self.on_stereotype_change).watch(
            "subject.appliedStereotype.slot", self.on_stereotype_attr_change
    ...
    def on_stereotype_attr_change(self, event):
        if (
            ...
            if isinstance(
                event, (UML.event.AssociationAddEvent, UML.event.AssociationDeleteEvent)
            ):
    ...
    def pre_update_compartment_icon(self, context):
        ...
        self.min_width = max(
            self.min_width, self._header_size[0] + self.ICON_WIDTH + 10
        )
    ...
    def get_icon_pos(self):
        ...
        Get icon position.
        """
        return self.width - self.ICON_MARGIN_X - self.ICON_WIDTH, self.ICON_MARGIN_Y
