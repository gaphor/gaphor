...
def load_elements(elements, factory, model_provider, gaphor_version="1.0.0"):
    for status in load_elements_generator(
        elements, factory, model_provider, gaphor_version
    ):
        pass


def load_elements_generator(elements, factory, model_provider, gaphor_version):
    """
    ...
    yield from _load_elements_and_canvasitems(
        elements, factory, model_provider, gaphor_version, update_status_queue
    )
...
def _load_elements_and_canvasitems(
    ...
    def create_canvasitems(diagram, canvasitems, parent=None):
        ...
        for item in canvasitems:
            ...
            cls = model_provider.lookup_diagram_item(item.type)
    ...
    for id, elem in list(elements.items()):
        ...
        if isinstance(elem, parser.element):
            cls = model_provider.lookup_element(elem.type)
...
def load(filename, factory, model_provider, status_queue=None):
    """
    Load a file and create a model if possible.
    Optionally, a status queue function can be given, to which the
    progress is written (as status_queue(progress)).
    """
    for status in load_generator(filename, factory, model_provider):
        if status_queue:
            status_queue(status)


def load_generator(filename, factory, model_provider):
    """
    Load a file and create a model if possible.
