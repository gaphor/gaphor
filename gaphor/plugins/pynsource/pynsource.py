...
class PySourceAsJava(PySourceAsText):
    ...
    def _OpenNextFile(self):
        filepath = f"{self.outdir}/{self.aclass}.java"
    ...
    def _DumpClassNameAndGeneralisations(self):
        ...
        self.result += f"public class {self.aclass} "
...
class PySourceAsDelphi(PySourceAsText):
    ...
    def _OpenNextFile(self):
        filepath = f"{self.outdir}/unit_{self.aclass}.pas"
        self.fp = open(filepath, "w")
    ...
    def _DumpAttribute(self, attrobj):
        ...
        if compositecreated:
            ...
            vartype = "Variant"
        self.result += f"{attrname} : {vartype};\n"
    ...
    def _DumpClassNameAndGeneralisations(self):
        ...
        self.result += "type\n\n"
        self.result += f"{self.aclass} = class"
        if self.classentry.classesinheritsfrom:
    ...
    def _DumpMethods(self):
        ...
        for adef in self.classentry.defs:
            if adef == "__init__":
                ...
                self.result += f"    procedure {adef}(); virtual;\n"
    ...
    def DumpImplementationMethods(self):
        for adef in self.classentry.defs:
            if adef == "__init__":
                self.result += (
                    f"constructor {self.aclass}.Create;\n"
                )  # replace __init__ with the word 'Create'
            else:
                ##                self.result +=  "function %s.%s(): void;\n" % (self.aclass, adef)
                self.result += f"procedure {self.aclass}.{adef}();\n"
            self.result += "begin\n"
            if adef == "__init__":
...
class PythonToJava:
    ...
    def _Process(self, filepath):
        ...
        if thefile[0] == "_":
            ...
            return
        print(f"{padding}Processing {thefile}...")
