...
def check_association_end_subsets(element_factory, end):
    ...
    opposite_subsets = get_subsets(
        end.opposite.taggedValue and end.opposite.taggedValue[0].value or ""
    )
    subset_properties = element_factory.select(
        lambda e: e.isKindOf(UML.Property) and e.name in subsets
    )
    ...
    for p in subset_properties:
        if p.opposite.name and p.opposite.name not in opposite_subsets:
            report(
                ...
                "Subset not defined on both sides. (%s, %s)"
                % (p.name, p.opposite.name),
            )
        ...
        if p.upperValue:
            if not end.upperValue:
                report(
                    ...
                    "Association end %s has no upper value, but the subset %s has"
                    % (end.name, p.name),
                )
            elif p.upperValue.value < end.upperValue.value:
                report(
                    end,
                    "Association end %s has has a bigger upper value than subse %s"
                    % (end.name, p.name),
                )
...
def check_attributes(element_factory):
    for a in element_factory.select(
        lambda e: e.isKindOf(UML.Property) and not e.association
    ):
