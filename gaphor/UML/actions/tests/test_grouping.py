...
def test_ungrouping_with_action_items(diagram, element_factory):
    """Test partition with actions removal."""
    p = diagram.create(
        PartitionItem, subject=element_factory.create(UML.ActivityPartition)
    )
    a1 = diagram.create(ActionItem, subject=element_factory.create(UML.Action))
    a2 = diagram.create(ActionItem, subject=element_factory.create(UML.Action))
    partition = p.subject
