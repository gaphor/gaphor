...
from properties import association, attribute, enumeration, derivedunion
"""
...
def write_derivedunion(d, out):
    ...
    if subs:
        out.write("%s.%s = derivedunion('%s', %s)\n" %
                  (d.class_name, d.name, d.name, subs))
    else:
        msg('no subsets for %s.%s' % (d.class_name, d.name))
        out.write("%s.%s = None # placeholder for empty unions\n" %
                  (d.class_name, d.name))
    d.written = True
    ...
    for a in filter(lambda e: hasattr(e, 'subsets'), associationends.values()):
        for s in a.subsets:
            try:
                ...
                msg('%s is not a derived union' % s)
